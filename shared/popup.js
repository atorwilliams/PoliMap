// popup.js — Popup/tooltip logic + member sidebars, config-driven
...
window.currentSidebar = currentSidebar;
...
export function showRidingPreview(map, ridingName, lngLat, level, config, onClose = null) {
  ...
  const dataUrl = level === 'federal' ? config.federalDataFile : config.dataFile;
  const memberKey = level === 'federal' ? (config.federalMemberKey || 'mp') : config.memberKey;
  const role = level === 'federal' ? config.federalMemberTitle : config.memberTitle;
  ...
  fetch(dataUrl)
    ...
    .then(data => {
      ...
      const official = ridingData[memberKey];
      ...
      const attachButton = () => {
        ...
        if (btn) {
          btn.addEventListener('click', (e) => {
            ...
            const member = ridingData[memberKey];
            if (member) {
              showMemberDetailSidebar(member, ridingName, level, data.electionDate, data.termEnd, config);
            }
          });
          return true;
        }
...
export function showPartySidebar(map, partyKey, members, level) {
...
function getPartyColorFromMembers(partyKey, members) {
  // Members carry their party info but not color — use known defaults
  const colors = {
    'NDP': '#F37021',
...
export function showMemberDetailSidebar(member, ridingName, level, electionDate, termEnd, config) {
  if (window.currentSidebar) {
    window.currentSidebar.remove();
    window.currentSidebar = null;
  }
