// rcmpLayer.js — RCMP detachment boundary layer, config-driven
...
function fixMultiRingPolygons(geojson) {
  return {
    ...
    features: geojson.features.map(feature => {
      ...
      if (exteriors.length <= 1) return feature;
...
let detachmentLookup = {};
let rcmpFeatures = [];

async function loadDetachmentLookup(detachmentsFile) {
  try {
    const res = await fetch(detachmentsFile);
...
export async function initRCMP(map, config) {
  await loadDetachmentLookup(config.rcmpDetachmentsFile);

  let geojson;
  try {
    const res = await fetch(config.rcmpBoundaryFile);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  map.addLayer({
    ...
    paint: { 'fill-color': DISTRICT_COLOR_EXPR, 'fill-opacity': 0.22 }
  });
  ...
  map.on('mouseenter', 'rcmp-fill', () => { map.getCanvas().style.cursor = 'pointer'; });
  map.on('mouseleave', 'rcmp-fill', () => { map.getCanvas().style.cursor = ''; });
...
function showRCMPSidebar(props, onClose) {
  ...
  const district = RCMP_DISTRICT_COLORS[props.GEO_BNDRY1] || RCMP_DISTRICT_COLORS.Other;
  const contract = props.Contract?.trim() ? `${props.Contract} Police` : 'K Division';
  ...
  sidebar.innerHTML = `
    ...
    <div class="member-info">
      ...
      <div class="contact-details">
        <div class="contact-item"><strong>Contract Type</strong><p>${contract}</p></div>
        ${address !== '—' ? `<div class="contact-item"><strong>Address</strong><p>${address}</p></div>` : ''}
        <div class="contact-item"><strong>Phone</strong><p>${phone}</p></div>
      </div>
    </div>
  `;
