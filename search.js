// search.js — Address search bar + "Your Representatives" results panel
...
export function initSearch(map) {
  ...
  const run = () => runSearch(map, input.value.trim(), btn, error);
...
async function runSearch(map, query, btn, errorEl) {
  if (!query) return;
  ...
  try {
    const url =
      ...
      `&countrycodes=ca&limit=1&viewbox=-120,60,-110,49&bounded=0`;
    ...
    if (!hits.length) {
      errorEl.textContent = 'Address not found. Try adding "Alberta, Canada".';
    ...
    // Drop a red marker and fly to it
    ...
    // PiP lookups — all boundary GeoJSON already in memory from init
    const provincialRiding = findProvincialRidingAt(lng, latNum);
    const federalRiding    = findFederalRidingAt(lng, latNum);
    const municipal        = findMunicipalAt(lng, latNum);
    const rcmp             = findRCMPAt(lng, latNum);

    // Load member/council data in parallel, only for what we found
...
function showResults(map, { display_name, provincialRiding, federalRiding, municipal, rcmp, provData, fedData, muniData }) {
  if (window.currentSidebar) {
    window.currentSidebar.remove();
    window.currentSidebar = null;
  }

  const mla   = provData?.ridings?.[provincialRiding]?.mla || null;
  const mp    = fedData?.ridings?.[federalRiding]?.mp      || null;
  const mayor = muniData?.mayor                            || null;

  const mlaColor = (mla?.party && provData?.parties?.[mla.party]?.color) || '#003DA5';
  ...
  sidebar.innerHTML = `
    ...
    <div style="height:4px; background:linear-gradient(90deg, ${mlaColor}, ${mpColor});"></div>
    <div class="member-info">
      <p class="riding">Your Representatives</p>
      <p class="search-result-address">${display_name}</p>
      <div class="search-rep-list">
        ${repCard({ color: mlaColor, label: 'Provincial MLA', riding: provincialRiding, person: mla, reptype: 'provincial' })}
        ${repCard({ color: mpColor,  label: 'Federal MP',     riding: federalRiding,    person: mp,  reptype: 'federal'    })}
  ...
  if (mla && provincialRiding) {
    sidebar.querySelector('[data-reptype="provincial"]')?.addEventListener('click', () => {
      sidebar.remove();
      window.currentSidebar = null;
      showMemberDetailSidebar(mla, provincialRiding, 'provincial', provData?.electionDate, provData?.termEnd);
    });
  }
  if (mp && federalRiding) {
    sidebar.querySelector('[data-reptype="federal"]')?.addEventListener('click', () => {
      sidebar.remove();
      window.currentSidebar = null;
      showMemberDetailSidebar(mp, federalRiding, 'federal', fedData?.electionDate, fedData?.termEnd);
    });
  }
...
function repCard({ color, label, riding, person, reptype }) {
  if (!riding) {
    return `<div class="search-rep-card" style="border-left-color:#333;">
      ...
      <span class="search-rep-none">Outside Alberta</span>
    </div>`;
  }
  if (!person) {
